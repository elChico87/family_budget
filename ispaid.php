...
<body>
  ...
  $fkispaid = $_SERVER['REQUEST_URI'];
  $pid = parse_url($fkispaid, PHP_URL_QUERY);
  $id = substr($pid,4);
  $aid = $connection->prepare("SELECT ispaid FROM BUD_PARAM_GROUP WHERE fk_bud_param = :fk_bud_param", array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
  $aid->bindValue('fk_bud_param', $id, PDO::PARAM_INT);
  $aid->execute();
...
​
<?php
$budget_list = $connection->prepare('UPDATE BUD_PARAM_GROUP SET ispaid = :ispaid WHERE fk_bud_param = :id');
$budget_list->bindValue(':id', $id, PDO::PARAM_INT);
$budget_list->bindValue(':ispaid', "Tak" , PDO::PARAM_STR);
$budget_list->execute();
echo "<p>Status został zmieniony!</p>";
