...
<body>
  ...
  $budget_list = $connection->prepare('SELECT p.name, pg.plan_value, pg.fk_bud_param, pg.value, pg.ispaid FROM BUD_PARAM AS p INNER JOIN BUD_PARAM_VALUE AS pg ON p.ID = pg.fk_bud_param WHERE pg.fk_budget = :name');
...
<?php
	...
	<tr>
		<td><?= $row['name']; ?></td>
		<td>
			<input type="text" class="form-control" name = "input[<?= $row['fk_bud_param']; ?>]" size="2">
		</td>
  <td><input type="text" class="form-control" name = "ispaid[<?= $row['fk_bud_param']; ?>]"  size="2" value="<?= $row['ispaid']; ?>">
</td>
	</tr>
	<?php
	}
...
<?php
        ...
        $chkbox = $connection->prepare("UPDATE BUD_PARAM_VALUE SET value = value + :value WHERE (fk_budget = :fk_budget) AND (fk_bud_param = :fk_bud_param)");
        foreach ($tablica_inputow as $fkBudParam => $value)
          {
          $chkbox->bindValue(':fk_budget', $highest_budget_id , PDO::PARAM_INT);
          $chkbox->bindValue(':value', $value, PDO::PARAM_INT);
          $chkbox->bindValue(':fk_bud_param',  $fkBudParam, PDO::PARAM_INT);
